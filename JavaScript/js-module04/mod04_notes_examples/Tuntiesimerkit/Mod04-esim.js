...
async function fetchAirport(icao) {
    ...
    const response = await fetch('http://localhost:3000/airport/' + icao);
    const airport = await response.json();

    // add data to dom
    const result = document.querySelector("#result");
    result.innerText = `ICAO: ${airport.ICAO}, name: ${airport.Name}`;
}
...
// käynnistä ensin 13-2.kopio -serveri, sitten avaa HTML-tiedosto selaimessa
